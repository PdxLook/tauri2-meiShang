  ...
  <main class="tags bg-black-200 h-full">
    <div class="table-card bg-white rounded space-y-5 p-5 h-full relative overflow-hidden">
      <div class="table-card-header space-y-5">
        ...
        <div class="left-button-group flex gap-2">
          <div class=""><el-button type="warning" size="small" @click="batchOperation('check')">批量审核</el-button></div>
          <div class=""><el-button type="danger" size="small" @click="batchOperation('reject')">批量退回</el-button></div>
          <div class=""><el-button type="danger" size="small" @click="batchOperation('delete')">批量删除</el-button></div>
        </div>
      </div>
...
const refreshTableData = () => {
  dataQuery.value = { ...dataQuery.value, ...searchQuery.value, refresh: true };
  getTableData(dataQuery.value);
}

const { batchOperation, deleteConfirm, getTableData } = useOperation(tableRef, funcOptions, refreshTableData)
